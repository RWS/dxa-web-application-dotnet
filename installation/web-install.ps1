...
Param(
    ...
    #Version of platform to run against "Web8" or "Sites9". Default is Sites9 and will configure web application to use PCA providers. Web8 option will use standalone model service.
    [Parameter(Mandatory=$false)]
    [ValidateSet("Web8", "Sites9")]
    [string]$platformVersion = "Sites9"
)
...
# Update Log.config
$logConfigFile = "$distDestination\NLog.config"
Write-Host ("Updating '$logConfigFile' ...")
[xml]$logConfig = Get-Content $logConfigFile -ErrorAction Stop
$targetNode = $logConfig.nlog.targets.target | ?{$_.name -eq "logfile"}
if ($targetNode) 
{ 
    $targetNode.SetAttribute("fileName", $siteLogFile)
    Write-Host "Set log file location to '$siteLogFile'" 
}
$logLevelNode = $logConfig.nlog.rules.logger
if ($logLevelNode)
{
    $logLevelNode.SetAttribute("minlevel", $logLevel)
...
{
    Set-CacheExpiration "regularCache" 15
    ...
    Set-AppSetting "viewModel-caching" $false
    Set-AppSetting "output-caching-enabled" $false
}
else
{
    ...
    Set-AppSetting "viewModel-caching" $true
    Set-AppSetting "output-caching-enabled" $true
...
if ($redisCacheServer)
{
    ...
    # Use Distributed Caching for all but DD4T cache Regions & navigation
    # We should be able to remove this once we target UDP with the new serialization changes included.
    $cilCacheSection.SelectNodes("regions/add[@cacheName='regularCache' and @name!='Page' and @name!='ComponentPresentation' and @name!='Navigation_Dynamic']") | ForEach-Object { $_.cacheName = "regularDistributedCache" }

    $cilCacheSection.SelectNodes("regions/add[@cacheName='longLivedCache' and @name!='RenderedOutput']") | ForEach-Object { $_.cacheName = "longLivedDistributedCache" }
...
function Remove-UnityTypeMapping([string] $type, [xml] $configDoc) 
{
    $mainContainer = $configDoc.unity.containers.container | ? {$_.name -eq "main"}
    if (!$mainContainer) 
    {
    ...
    if ($typeElement)
    {
        Write-Host "Found type mapping to remove."
        $typeElement.ParentNode.RemoveChild($typeElement)
    }
}

function Get-Namespace([string] $namespace, [xml] $configDoc)
{
    $unity = $configDoc.unity
    if (!$unity) 
    {
        throw "Unity element not found."
    }
    $namespaces = $unity.GetElementsByTagName("namespace")
...
function Remove-Namespace([string] $namespace, [xml] $configDoc)
{
    $ns = Get-Namespace $namespace $configDoc
    if($ns)
    {
...
function Add-Namespace([string] $namespace, [xml] $configDoc)
{
    $ns = Get-Namespace $namespace $configDoc
    if(!$ns)
    {       
        $unity = $configDoc.unity
        $nsElement = $configDoc.CreateElement("namespace")
        $nsElement.SetAttribute("name", $namespace)
        $unity.AppendChild($nsElement)
    }
}
...
if ($platformVersion -eq "Sites9")
{
    Set-UnityTypeMapping "ILocalizationResolver" "GraphQLLocalizationResolver" $unityConfigDoc
    Set-UnityTypeMapping "IModelServiceProvider" "GraphQLModelServiceProvider" $unityConfigDoc
    Set-UnityTypeMapping "IContentProvider" "GraphQLContentProvider" $unityConfigDoc
    Set-UnityTypeMapping "ILinkResolver" "GraphQLLinkResolver" $unityConfigDoc
    Set-UnityTypeMapping "IBinaryProvider" "GraphQLBinaryProvider" $unityConfigDoc
    Add-Namespace "Sdl.Web.Tridion.Providers.Binary" $unityConfigDoc
}
else
{
    Set-UnityTypeMapping "ILocalizationResolver" "CdApiLocalizationResolver" $unityConfigDoc
    Set-UnityTypeMapping "IModelServiceProvider" "DefaultModelServiceProvider" $unityConfigDoc
    Set-UnityTypeMapping "IContentProvider" "DefaultContentProvider" $unityConfigDoc
    Set-UnityTypeMapping "ILinkResolver" "DefaultLinkResolver" $unityConfigDoc
    Remove-UnityTypeMapping "IBinaryProvider" $unityConfigDoc
    Remove-Namespace "Sdl.Web.Tridion.Providers.Binary" $unityConfigDoc
}
